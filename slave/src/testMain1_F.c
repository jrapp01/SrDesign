...
typedef enum{
    ...
    GET_NEXT    //State to denoise, wait until signal below threshold
}state;
...
int velocity[16][2]={0};        //Extract note-on velocity
int forceData[16][2]={0};       //Collect FSR hammer force data
int slopeData[16][2]={0};       //Contain calculated slope data
...
void SendNotes(int note){
    ...
    if((txFlag & (1<<note))>>note){
        ...
        //write note number to master
        mPMPSlaveWrite(note);
        while(!mIsPMPSlaveBufferEmpty());
        
        //write note velocity to master
...
void noteOn(int note, int velocity){
    ...
    //velocity scaling based on input dynamic range
    velocity = (velocity-247)/3;
    
    //if value over 127, hard limit to max velocity
...
void __ISR(_CHANGE_NOTICE_VECTOR, ipl2) ChangeNotice_Handler(void)
{
    ...
    int dummy0 = PORTReadBits(IOPORT_D, BIT_6);
    int dummy1 = PORTReadBits(IOPORT_D, BIT_7);
...
void __ISR(_EXTERNAL_0_VECTOR, IPL6) Note2_Off(void){
    noteOff(2);
    mPORTDToggleBits(BIT_1);
    mINT0ClearIntFlag();
}

void __ISR(_EXTERNAL_1_VECTOR, IPL6) Note5_Off(void){
    noteOff(5);
    mPORTDToggleBits(BIT_1);
    mINT1ClearIntFlag();
}

void __ISR(_EXTERNAL_2_VECTOR, IPL6) Note7_Off(void){
    noteOff(7);
    mPORTDToggleBits(BIT_1);
    mINT2ClearIntFlag();
}

void __ISR(_EXTERNAL_3_VECTOR, IPL6) Note10_Off(void){
    noteOff(10);
    mPORTDToggleBits(BIT_1);
    mINT3ClearIntFlag();
}